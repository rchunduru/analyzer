...
(function() {
  var AnalyzerManager, AnalyzerService, AnalyzerServices, EA, EM, MM, SA, SD, SR, async, bunyan, parseMessage, parsequery, parseurl, postRequest, promise, uuid, validate,
  ...
  AnalyzerService = (function(_super) {
      ...
      if (this.data && (this.data.sources != null)) {
        return this.data.sources.map((function(_this) {
          return function(source) {
            ...
            _this.log("Debug: parsedurl query is ", parsedurl.query);
    ...
    AnalyzerService.prototype.subscribe = function() {
      ...
      loggersyslog = (function(_this) {
        return function(message) {
          var bodykey, cname, data, msg, objkey, str, strarray, _i, _len;
          ...
          switch (msg.header.format.toLowerCase()) {
            ...
            default:
              data = msg.data;
              if (!((data.count != null) && (data.start != null))) {
                return;
              }
    ...
    AnalyzerService.prototype.mailvirus = function(emailData) {
      return this.em.createDocument(this.eclient, this.id, 'email.virus', emailData).then((function(_this) {
        return function(response) {
          _this.log("Added email virus ", response);
          return _this.emit('email.virus', _this.id, emailData);
        };
      })(this), (function(_this) {
    ...
    AnalyzerService.prototype.cleanElasticDocuments = function() {
      return new promise((function(_this) {
        return function(fulfill, reject) {
          ...
          return _this.em.deleteAllDocumentsAsync(_this.eclient, _this.id, 'email.virus').then(function(response) {
            _this.log("Deleted all documents of type email.virus");
            return _this.em.deleteAllDocumentsAsync(_this.eclient, _this.id, 'transaction.summary').then(function(response) {
    ...
    AnalyzerService.prototype.createTransaction = function(content) {
      return this.em.createDocument(this.eclient, this.id, 'transaction.summary', content).then(function(result) {
    ...
    AnalyzerService.prototype.updateTransaction = function(id, content) {
      return this.em.updateDocument(this.eclient, this.id, 'transaction.summary', content).then(function(result) {
    ...
    AnalyzerService.prototype.getTransaction = function() {
      ...
      return new promise((function(_this) {
        return function(fulfill, reject) {
          return _this.em.search(_this.eclient, _this.id, 'transaction.summary', body).then(function(results) {
            if (results.length > 1) {
      ...
      this.on('load', function(key, val) {
        ...
        if (entry != null) {
          ...
          return entry.on('email.virus', (function(_this) {
            return function(id, emailData) {
              return _this.emit('email.virus', id, emailData);
            };
          })(this));
        }
      });
  ...
  AnalyzerManager = (function(_super) {
    ...
    AnalyzerManager.prototype.run = function(config) {
      ...
      return this.aservices.on('email.virus', (function(_this) {
        return function(id, emailData) {
          _this.log("Debug: Notifying USG " + _this.config.usgEmailNotify + " with email Data for instance" + id);
          return _this.notifyUSG(emailData).then(function(response) {
            return _this.log("Successfully updated the USG", response);
          }, function(error) {
