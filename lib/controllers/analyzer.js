  ...
  AnalyzerService = (function(_super) {
    ...
    AnalyzerService.prototype.dbConnect = function() {
      ...
      if (this.data && (this.data.output != null)) {
        ...
        /*
         * XXX Hack - for cleaning up DB
        @em.deleteAllDocuments @eclient, "email.virus", @id
        . then (resp) ->
            console.log "result for deleting", resp
        @em.deleteAllDocuments @eclient, @id, "email.virus"
        . then (resp) ->
            console.log "result for deleting", resp
    ...
    AnalyzerService.prototype.createTransaction = function(content) {
      return new promise(function(fulfill, reject) {
        return this.em.createDocument(this.eclient, 'transaction.summary', this.id, content).then((function(_this) {
    ...
    AnalyzerService.prototype.updateTransaction = function(id, content) {
      return this.em.updateDocument(this.eclient, 'transaction.summary', this.id, content).then((function(_this) {
        return function(result) {
      ...
      return async.whilst(function() {
        ...
        return function(searchloop) {
          ...
          _this.em.search(_this.eclient, 'email.virus', _this.id, body).then(function(results) {
            _this.log("Number of Results are ", results.length);
            if ((results != null) && results.length === 0) {
              _this.log("no results found");
              return;
            }
            return promise.all(results.hits.map(function(result) {
          ...
          return setTimeout(searchloop, 200000);
        };
      })(this), (function(_this) {
        return function(err) {
