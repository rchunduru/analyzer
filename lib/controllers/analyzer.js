...
(function() {
  ...
  AnalyzerService = (function(_super) {
    ...
    AnalyzerService.prototype.subscribe = function() {
      var b, topic, _i, _len, _ref, _results;
      _ref = this.topics;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        topic = _ref[_i];
        _results.push((this.subscriptions != null ? this.subscriptions : this.subscriptions = {})[b = topic] = this.mq.subscribe("/topic/" + topic, this.loggersyslog));
      }
      return _results;
    };
    ...
    AnalyzerService.prototype.loggersyslog = function(message) {
      var bodykey, buf, data, msg, objkey, str, strarray, syslog, _i, _len;
      this.log("Assuming string format rcvd from MQ", message);
      buf = new Buffer(message.body.length);
      msg = this.ea.stripHeader(buf);
    ...
    AnalyzerService.prototype.mailvirus = function(emailData) {
      return this.eclient.createDocumentAsync(this.id, 'email.virus', emailData).then(function(response) {
        this.log("Added email virus ", response);
        return this.emit('email.virus', emailData);
      }, function(error) {
  ...
  AnalyzerServices = (function(_super) {
        ...
        if (entry != null) {
          ...
          return entry.on('email.virus', (function(_this) {
            return function(emailData) {
              return _this.emit('email.virus', emailData);
            };
          })(this));
        }
      });
    ...
    AnalyzerServices.prototype.remove = function(key) {
      ...
      entry.cleanup();
      return AnalyzerServices.__super__.remove.call(this, key);
    };
  ...
  AnalyzerManager = (function(_super) {
    ...
    AnalyzerManager.prototype.run = function(config) {
      ...
      return this.aservices.on('email.virus', (function(_this) {
        return function(emailData) {
          return _this.notifyUSG(emailData);
        };
      })(this));
    };
